...
If `vs_prior` is missing or empty, the household doesn't have enough history yet. Tell the user how much data they have and bow out: "I need at least a year and some change to see drift. Come back in a few months."
...
- **Drift gap < −3 pp** — the user moved the other way. Show the picture and call out where the gain came from. Often one big change: paid off a car, downsized housing, sold a business. Name it.
...
### Step 6: Flag possible one-time events

If a group jumped from near zero to a material number (or the reverse), say so. A wedding, a move, a baby, a medical event, a one-time tax bill, a paid-off car will look like steady drift but isn't. The user knows; let them confirm rather than treating the spike as ongoing.
...
- **One headline sentence.** Income up X%, expenses up Y%, gap is Z percentage points. Or the appropriate triage variant.
- **A short table.** 3–5 biggest movers — group, prior, current, dollar change, percent change.
- **Callouts.** One-time events, income drops, paid-off debt, savings rate flat-vs-rising-income, anything that changes how the table should be read.
- **A monthly-drag line** if drift is positive: "$X/mo of new ongoing spending."
